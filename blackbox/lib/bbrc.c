...
static struct lin_list *search_line(
    struct fil_list *fl, const char *key, int fwild, LONG *p_seekpos);

/* ------------------------------------------------------------------------- */

static int translate_key070(char *key)
{
...
/* ------------------------------------------------------------------------- */
static bool translate_key065(char *key)
{
...
static struct lin_list *search_line_065(struct fil_list *fl, const char *key)
{
...
static void write_rcfile(struct fil_list *fl)
{
    FILE *fp;
    unsigned ml = 0;
    struct lin_list *tl;
...
static void mark_rc_dirty(struct fil_list *fl)
{
    fl->dirty = true;
}

/* ------------------------------------------------------------------------- */
static void delete_lin_list(struct fil_list *fl)
{
    freeall(&fl->lines);
...
static void delete_fil_list(struct fil_list *fl)
{
    if (fl->dirty)
        write_rcfile(fl);
    delete_lin_list(fl);
...
static VOID CALLBACK reset_reader_proc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
    if (g_rc) {
        if (g_rc->used) {
            g_rc->used = 0;
...
static void set_reader_timer(void)
{
    if (g_rc->timer_set)
        return;
...
static void del_from_list(void *tlp, void *tl, void *n)
{
...
static int simkey(const char *a0, const char *b0)
{
