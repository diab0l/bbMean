    ...
    int MouseAction(HWND hwnd, unsigned int message, WPARAM wParam, POINT *p, int index)
    {
        ...
        if (icon)
        {
            ...
            if (message == WM_LBUTTONUP) {
				SFGAOF isFolderFlag = SFGAO_FOLDER;

				struct IShellFolder* pShellFolder = sh_get_folder_interface(pidl);
				char menuName[260];
				char path[260];

				if (pShellFolder 
					&& (S_OK == pShellFolder->GetAttributesOf(1, &pidl, &isFolderFlag)) 
					&& isFolderFlag
					&& sh_getnameof(NULL, pidl, SHGDN_FORPARSING, path))
				{
					if(!sh_getnameof(NULL, pidl, SHGDN_NORMAL, menuName))
						memcpy(menuName, path, 260);
					
					Menu* folderMenu = MakeNamedMenu(menuName, "bbIconBox.folderMenu", true);
					
					struct IEnumIDList* menuItemEnum;
					LPITEMIDLIST item;
					ULONG fetched;
					
					if(S_OK == pShellFolder->EnumObjects(BBhwnd, SHCONTF_FOLDERS, &menuItemEnum))
					{
						while(S_OK == menuItemEnum->Next(1, &item, &fetched) && fetched != 0) {
							sh_getnameof(pShellFolder, item, SHGDN_NORMAL, menuName);
							sh_getnameof(pShellFolder, item, SHGDN_FORPARSING, path);
							MakeMenuItemPath(folderMenu, menuName, path);
						}
						menuItemEnum->Release();
					}

					if(S_OK == pShellFolder->EnumObjects(BBhwnd, SHCONTF_NONFOLDERS, &menuItemEnum))
					{
						while(S_OK == menuItemEnum->Next(1, &item, &fetched) && fetched != 0) {
							sh_getnameof(pShellFolder, item, SHGDN_NORMAL, menuName);
							sh_getnameof(pShellFolder, item, SHGDN_FORPARSING, path);
							MakeMenuItem(folderMenu, menuName, path, false);
						}
						menuItemEnum->Release();
					}

					BBP_showmenu(this, folderMenu, true|BBMENU_CORNER);
