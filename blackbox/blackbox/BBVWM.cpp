...
static BOOL CALLBACK win_enum_proc(HWND hwnd, LPARAM lParam)
{
    vwm_add_window(hwnd);
    return TRUE;
}
...
static void fix_window(int *left, int *top, int width, int height)
{
    RECT x, d, w;
    int x0, y0, dx, dy;
...
static void center_window(int *left, int *top, int width, int height)
{
    int x0, dx, w2;
...
static void add_workarea(RECT *p, int d)
{
...
static bool set_normal_position(HWND hwnd, RECT *p)
{
    WINDOWPLACEMENT wp;
    wp.length = sizeof wp;
...
static bool get_normal_position(HWND hwnd, RECT *p)
{
    WINDOWPLACEMENT wp;
    wp.length = sizeof wp;
    if (!GetWindowPlacement(hwnd, &wp))
...
static bool belongs_to_app(winlist *win, winlist *app)
{
...
static void check_appwindows(winlist *wl_app)
{
    winlist *wl;
    if (wl_app->istool)
        dolist (wl, vwm_WL)
...
//=========================================================
static void defer_windows(int newdesk)
{
    winlist *wl;
    HDWP dwp, dwp_new;
...
static void explicit_move(winlist *wl)
{
...
static bool set_location_helper(HWND hwnd, struct taskinfo *t, unsigned flags)
{
    winlist *wl;
