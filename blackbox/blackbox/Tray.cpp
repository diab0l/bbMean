...
static void tray_notify(systemTrayNode *p, UINT message)
{
    if (p->version >= 4) {
...
static int forward_tray_message(systemTrayNode *p, UINT message, systemTrayIconPos *pos)
{
    HWND hwnd;
    systemTrayNode *q;
...
static void reset_icon(systemTrayNode *p)
{
    if (false == p->shared) {
        systemTrayNode *s;
...
static LRESULT send_tray_message(systemTrayNode *p, unsigned uChanged, unsigned msg)
{
...
static bool convert_string(char *dest, const void *src, int nmax, bool is_unicode)
{
    char buffer[256];
    if (is_unicode) {
...
static void log_tray(DWORD trayCommand, NIDBB *pnid)
{
...
static LRESULT TrayEvent(void *data, unsigned size)
{
...
//===========================================================================
static LRESULT TrayInfoEvent(void *data, unsigned size)
{
...
static LRESULT TrayTestEvent(void *data, unsigned size)
{
#if 0
    char s_guid[40];
...
static int sso_load(const char *name, const char *guid)
{
    WCHAR wszCLSID[200];
    CLSID clsid;
...
static DWORD WINAPI SSO_Thread(void *pv)
{
    HKEY hk0, hk1;
    sso_list_t *t;
    const char *key;
...
static void broadcast_tbcreated(void)
{
...
static HWND create_tray_child(HWND hwndParent, const char *class_name)
{
