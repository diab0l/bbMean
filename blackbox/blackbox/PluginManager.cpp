...
static void free_plugin(struct plugins **pp)
{
    struct plugins *q = *pp;
    if (q) {
        free_str(&q->name);
...
static void check_plugin(struct plugins *q)
{
    int error = 0;
    if (q->hmodule) {
...
static Menu *get_menu(const char *title, char *menu_id, bool pop, struct plugins **qp, bool b_slit)
{
    struct plugins *q;
    char *end_id;
    MenuItem *pItem;
