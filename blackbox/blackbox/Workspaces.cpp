...
//===========================================================================
static HWND get_default_window(HWND hwnd)
{
    if (NULL == hwnd) {
...
static LRESULT send_syscommand(HWND hwnd, WPARAM SC_XXX)
{
...
static DWORD_PTR send_bbls_command(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    DWORD_PTR result = 0;
...
static BOOL list_desktops_func(DesktopInfo *DI, LPARAM lParam)
{
...
/*
static void post_message_if_needed(UINT message, WPARAM wParam, LPARAM lParam)
{
    MSG msg;
...
static void send_task_message(HWND hwnd, UINT msg)
{
...
static int get_shade_height(HWND hwnd)
{
...
static void WS_RaiseWindow(HWND hwnd_notused)
{
...
static bool mr_checktask(HWND hwnd)
{
    struct tasklist *tl = (struct tasklist *)assoc(taskList, hwnd);
    return tl && tl->wkspc == currentScreen;
}

static BOOL CALLBACK mr_enumproc(HWND hwnd, LPARAM lParam)
{
...
static void min_rest_helper(int cmd)
{
...
static void switchToDesktop(int n)
{
...
static void setDesktop(HWND hwnd, int n, bool switchto)
{
...
static void MoveWindowToWkspc(HWND hwnd, int desk, bool switchto)
{
    if (NULL == hwnd)
        return;
...
static void NextWindow(bool allDesktops, int dir)
{
...
static void del_from_toptasks(struct tasklist *tl)
{
...
static void SetTopTask(struct tasklist *tl, int set_where)
{
    if (NULL==tl)
        return;
...
static void get_caption(struct tasklist *tl, int force)
{
...
static struct tasklist *AddTask(HWND hwnd)
{
...
static void RemoveTask(struct tasklist *tl)
{
    HWND hwnd;
    if (tl->icon)
        DestroyIcon(tl->icon);
...
//==================================
static BOOL CALLBACK TaskProc(HWND hwnd, LPARAM lParam)
{
    if (IsAppWindow(hwnd))
...
//===========================================================================
#if 0
static void debug_tasks(WPARAM wParam, HWND hwnd, int is_task)
{
